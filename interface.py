...
class DataTracker:
    ...
    def __init__(self, root):
        ...
        try:
            ...
            self.conn = None
            self.cursor = None  # Убедимся, что соединение не установлено
        ...
        self.current_user_role = None
            ...
            # Заголовок
        self.label = tk.Label(root, text="DataTracker", font=("Arial", 16), bg="#F0F0F0")
        self.label.grid(row=0, column=0, columnspan=2, pady=10)

            # Ввод уровня
        self.level_label = tk.Label(root, text="Введите уровень:", font=("Arial", 14), bg="#F0F0F0")
        self.level_label.grid(row=1, column=0, sticky='w', padx=10)

        self.level_entry = tk.Entry(root, bg="#D9D9D9", font=("Arial", 14))
        self.level_entry.grid(row=1, column=1, padx=10, sticky='ew')
        self.level_entry.config(state='disabled')  # Отключаем поле ввода

            # Ввод очков
        self.score_label = tk.Label(root, text="Введите количество очков:", font=("Arial", 14), bg="#F0F0F0")
        self.score_label.grid(row=2, column=0, sticky='w', padx=10)

        self.score_entry = tk.Entry(root, bg="#D9D9D9", font=("Arial", 14))
        ...
        self.add_button = tk.Button(root, text="Добавить данные", command=self.add_data,
                                        width=20, height=2, bg="#c0c0c0", fg="black", font=("Arial", 12))
        ...
        self.analyze_button = tk.Button(root, text="Анализировать данные", command=self.analyze_data,
                                            width=20, height=2, bg="#b0b0b0", fg="black", font=("Arial", 12))
        ...
        self.achievements_button = tk.Button(root, text="Просмотр достижений", command=self.view_achievements,
                                                 width=20, height=2, bg="#a0a0a0", fg="black", font=("Arial", 12))
        ...
        self.login_button = tk.Button(root, text="Авторизация пользователя", command=self.open_login_window,
                                          width=20, height=2, bg="#32CD32", fg="black", font=("Arial", 12))
        self.login_button.grid(row=6, column=0, columnspan=2, pady=(5, 10), sticky='ew', padx=(20, 20))
        ...
        self.register_button = tk.Button(root, text="Регистрация пользователя",
                                             ...
                                             width=20, height=2, bg="#FFD700", fg="black", font=("Arial", 12))
        self.register_button.grid(row=7, column=0, columnspan=2, pady=(5, 10), sticky='ew', padx=(20, 20))

            # Кнопка для экспорта данных
        self.export_button = tk.Button(root, text="Экспорт данных", command=self.export_user_data, width=20)
        self.export_button.grid(row=0, column=1, pady=(5, 5), padx=(5, 5), sticky='ne')

    def export_user_data(self):# Экспорт данных пользователя с максимальным уровнем в текстовый файл.
    ...
    def view_achievements(self):
        ...
        achievements_window.geometry('800x500')
        ...
        def load_achievements(search_query=None):
            ...
            if search_query:
                ...
                """
                self.cursor.execute(query_string, (self.current_user_id, f"%{search_query}%"))
            else:
                query_string = """
                    SELECT a.name, a.description, a.criteria, ua.date
                    FROM achievements a
                    ...
                    WHERE ua.user_id = ?
                """
                self.cursor.execute(query_string, (self.current_user_id,))
        ...
        search_button = tk.Button(
            ...
            text="Найти",
            font=("Arial", 12),
            command=lambda: load_achievements(search_entry.get())
        )
        search_button.pack(pady=5)

        # Загрузка всех достижений при открытии окна
        load_achievements()
    ...
    def create_admin_buttons(self):# Создание кнопки 'Изменить базу данных' для администраторов.
        ...
        db_button = tk.Button(self.root, text="Изменить базу данных", command=self.open_database_menu, width=20, height=2, bg="#FF4500", fg="black", font=("Arial", 12))
        db_button.grid(row=9, columnspan=2, pady=(5, 10), sticky='ew', padx=(20, 20))
